...
use Admin\MedicalBundle\Form\CategoryPriceType;
    ...
    public function addeditAction(Request $request)
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                ...
                if (($asData['client_medicalbundle_category_price']['categories']) > 0) {
                    $priceData = $asData['client_medicalbundle_category_price'];
                    foreach ($priceData['categories'] as $key => $category) {
                        $companyCategory = new CompanyCategory();
                        $company = $em->getRepository('AdminMedicalBundle:Company')->find($snId);
                        $categoryObj = $em->getRepository('AdminMedicalBundle:Category')->find($category);
                        $categoryRoot = $em->getRepository('AdminMedicalBundle:Category')->find($categoryObj->getRootId());
                        $companyCategory->setCategories($categoryObj);
                        $companyCategory->setRoots($categoryRoot);
                        $companyCategory->setCompany($company);
                        $companyCategory->setMinprice($priceData['minprice'][$key]);
                        $companyCategory->setMaxprice($priceData['maxprice'][$key]);

                        $em->persist($companyCategory);
                        $em->flush($companyCategory);
                    }
